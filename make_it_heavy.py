...
class OrchestratorCLI:
    def __init__(self):
        ...
        model_full = self.orchestrator.config["ollama"]["model"]
        # Extract model name (e.g., "llama3.2:3b" -> "LLAMA3.2")
        if ":" in model_full:
        ...
        # Take first 2 parts for cleaner display (e.g., llama3.2)
        clean_name = ".".join(model_parts[:2]) if len(model_parts) >= 2 else model_name
    ...
    def interactive_mode(self):
        ...
        try:
            orchestrator_config = self.orchestrator.config["ollama"]
